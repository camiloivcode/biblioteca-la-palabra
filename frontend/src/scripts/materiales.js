...
function estadoBadge(estado) {
  const map = { DISPONIBLE: 'badge-disponible', PRESTADO: 'badge-prestado', EN_REPARACION: 'badge-mora', DADO_DE_BAJA: 'badge-suspendido' };
  return `<span class="status-badge ${map[estado] || ''}">${estado.replace('_', ' ')}</span>`;
}
...
async function cargarMateriales(page = 1, filters = {}) {
  ...
  try {
    const res = await fetch(`${API}/materiales?${params}`, { headers: headers() });
    const data = await res.json();
    if (!data.success) throw new Error(data.message);

    const tbody = document.getElementById('tbody-materiales');
    const { data: materiales, meta } = data;

    if (!materiales.length) {
      tbody.innerHTML = '<tr><td colspan="9" class="text-center py-5 text-muted">No se encontraron materiales</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = materiales.map((m, i) => `
      <tr>
        ...
        <td>
          <span class="badge bg-light text-dark" style="font-size:11px;">${m.tipo}</span>
        </td>
        <td>${m.autor ? `${m.autor.apellido}, ${m.autor.nombre}` : '—'}</td>
        <td>${m.categoria?.nombre || '—'}</td>
        <td class="text-muted-sm">${m.isbn || '—'}</td>
        <td>
          <span class="fw-semibold">${m.stock}</span>
          ${m.anioPubl ? `<div class="text-muted-sm">${m.anioPubl}</div>` : ''}
        </td>
        <td>${estadoBadge(m.estado)}</td>
        <td>
          <div class="d-flex gap-1 justify-content-center">
            <button class="btn btn-icon btn-sm btn-outline-primary" onclick="editarMaterial(${m.id})" title="Editar">
              <i class="bi bi-pencil"></i>
            </button>
            <button class="btn btn-icon btn-sm btn-outline-danger" onclick="eliminarMaterial(${m.id}, '${m.titulo.replace(/'/g, "\\'")}')" title="Eliminar">
    ...
    document.getElementById('pag-mat-info').textContent = `Mostrando ${materiales.length} de ${meta.total} materiales`;
    ...
    document.getElementById('tbody-materiales').innerHTML =
      `<tr><td colspan="9" class="text-center py-4 text-danger">${err.message}</td></tr>`;
  }
}

async function cargarSelectsMaterial() {
  const [autRes, catRes] = await Promise.all([
    fetch(`${API}/autores`, { headers: headers() }),
...
async function editarMaterial(id) {
  try {
    ...
    const res = await fetch(`${API}/materiales/${id}`, { headers: headers() });
    const data = await res.json();
    if (!data.success) throw new Error(data.message);
...
async function eliminarMaterial(id, titulo) {
  ...
  try {
    showLoader();
    const res = await fetch(`${API}/materiales/${id}`, { method: 'DELETE', headers: headers() });
    const data = await res.json();
    if (!data.success) throw new Error(data.message);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('btn-guardar-material')?.addEventListener('click', async function () {
    ...
    try {
      ...
      const url = isEdit ? `${API}/materiales/${editingId}` : `${API}/materiales`;
      ...
      modal.hide();
      Swal.fire({ icon: 'success', title: isEdit ? 'Material actualizado' : 'Material registrado', timer: 1500, showConfirmButton: false });
      cargarMateriales(currentPage, currentFilters);
    } catch (err) {
      Swal.fire({ icon: 'error', title: 'Error', text: err.message });
    } finally {
